...
# ── Current / History archive ─────────────────────────────────────────────────

def _save_to_current(src_ply):
    """
    Write src_ply to z_Current Point cloud with Y negated (camera Y is down;
    negate so the saved file has Y pointing up — correct orientation in viewers).
    Archives any existing current_pointcloud.ply to history first.
    """
    import shutil
    workspace    = os.path.dirname(ROOT)
    current_dir  = os.path.join(workspace, "z_Current Point cloud")
    history_dir  = os.path.join(workspace, "z_History Point Cloud")
    fixed_name   = "current_pointcloud.ply"
    current_file = os.path.join(current_dir, fixed_name)
    ...
    if os.path.exists(current_file):
        ...
        hist_file = os.path.join(history_dir, f"pointcloud_{ts}.ply")
        shutil.move(current_file, hist_file)
        print(f"  Archived previous  -> {hist_file}")

    shutil.copy2(src_ply, current_file)
    print(f"  Current point cloud -> {current_file}")


def _save_to_current_heatmap(src_png):
    """
    Copy src_png to z_Current Heatmap_PNG as a fixed filename.
    If a file already exists there, move it to z_History Heatmap_PNG first.
    """
    if not src_png or not os.path.exists(src_png):
        return
    import shutil
    workspace    = os.path.dirname(ROOT)
    current_dir  = os.path.join(workspace, "z_Current Heatmap_PNG")
    history_dir  = os.path.join(workspace, "z_History Heatmap_PNG")
    fixed_name   = "current_heatmap.png"
    current_file = os.path.join(current_dir, fixed_name)
    ...
    if os.path.exists(current_file):
        ts        = datetime.now().strftime("%Y%m%d_%H%M%S")
        hist_file = os.path.join(history_dir, f"heatmap_{ts}.png")
        shutil.move(current_file, hist_file)
        print(f"  Archived previous  -> {hist_file}")
    ...
    if img is not None:
        img = cv2.flip(img, 1)  # flip along vertical axis (left-right mirror)
        cv2.imwrite(current_file, img)
    else:
        shutil.copy2(src_png, current_file)
    print(f"  Current heatmap    -> {current_file}")


# ── Point cloud → mesh ────────────────────────────────────────────────────────

MESH_PY    = os.path.join(os.path.dirname(ROOT),
                "05_PointCloud to Mesh",
                "Clay-Create-pointcloud-to-3d",
                "pointcloud_to_mesh", "mesh.py")
PYTHON_312 = os.environ.get("MESH_PYTHON_EXE") or sys.executable


def _mesh_from_pointcloud(src_ply):
    """Run mesh.py via Python 3.12 (Open3D) and archive result to z_Current Point Cloud_Mesh."""
    import shutil
    import subprocess

    workspace    = os.path.dirname(ROOT)
    current_dir  = os.path.join(workspace, "z_Current Point Cloud_Mesh")
    history_dir  = os.path.join(workspace, "z_History Point Cloud_Mesh")
    fixed_name   = "current_mesh.obj"
    current_file = os.path.join(current_dir, fixed_name)
    ...
    if os.path.exists(current_file):
        ts        = datetime.now().strftime("%Y%m%d_%H%M%S")
        hist_file = os.path.join(history_dir, f"mesh_{ts}.obj")
        shutil.move(current_file, hist_file)
        print(f"  Archived previous mesh -> {hist_file}")

    print("  [Mesh] Running Poisson reconstruction (Python 3.12)...")
    result = subprocess.run(
        [PYTHON_312, MESH_PY, "--input", src_ply, "--output", current_file],
        capture_output=False,
    )
    if result.returncode != 0:
        print(f"  [Mesh] mesh.py exited with code {result.returncode}")
    elif os.path.exists(current_file):
        print(f"  Current mesh -> {current_file}")
    else:
        print("  [Mesh] mesh.py finished but output file not found.")
...
def process(ply_path, open_viewer=True,
    ...
    # Flip Y (horizontal axis) then rotate 90° clockwise before saving
    points[:, 1] = -points[:, 1]
    ...
    _save_to_current(out_ply)
    _save_to_current_heatmap(color_png)

    print("\nStep 4 — orthographic projections")
...
if __name__ == "__main__":
    ...
    if args.capture:
        ...
        py  = sys.executable
        ts  = datetime.now().strftime("%Y%m%d_%H%M%S")
        ply = os.path.join(ROOT, "point clouds", f"capture_{ts}.ply")
