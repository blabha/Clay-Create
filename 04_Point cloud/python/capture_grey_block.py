...
def _load_roi_config():
    """Load bounding box and pixel ROI from roi_config.txt saved by live_roi.py, if present."""
...
def main():
    ...
    pixel_roi = None
    if all(k in roi for k in ("roi_u0", "roi_v0", "roi_u1", "roi_v1")):
        pixel_roi = (int(roi["roi_u0"]), int(roi["roi_v0"]),
                     int(roi["roi_u1"]), int(roi["roi_v1"]))
        print(f"  Pixel ROI mask: ({pixel_roi[0]},{pixel_roi[1]}) → ({pixel_roi[2]},{pixel_roi[3]})")
