...
async def master_png():
    """Serve the current master heightmap as a PNG (for external consumers)."""
    if not tile_manager.initialized or tile_manager.master is None:
        raise HTTPException(status_code=404, detail="Not initialized")
    img = Image.fromarray((np.clip(tile_manager.master, 0, 1) * 255).astype(np.uint8))
    buf = io.BytesIO()
    img.save(buf, format="PNG")
    buf.seek(0)
...
@app.post("/api/tile/{idx}/save-target")
async def save_target(idx: int):
    """Save the tile's intended design as current_target.jpg in z_CurrentTargetDesign."""
    ...
    workspace    = Path(__file__).resolve().parents[3]   # Hardware 3 root
    current_dir  = workspace / "z_CurrentTargetDesign"
    done_dir     = workspace / "z_Completed Target Design"
    target_file  = current_dir / "current_target.jpg"
    ...
    if target_file.exists():
        ...
        shutil.move(str(target_file), str(done_dir / f"target_{ts}.jpg"))

    img = Image.fromarray((np.clip(tile, 0, 1) * 255).astype(np.uint8)).convert("RGB")
    img.save(str(target_file), "JPEG", quality=95)
    print(f"[Target] Tile {idx} saved -> {target_file}")
    return {"saved": str(target_file)}
