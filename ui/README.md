...
# gh\_state.json (shared state file)
...
# This GHPython component reads the session state from `gh\_state.json` and outputs:
...
# \*\*Important:\*\* Update the `FILE\_PATH` variable in the script to match your local path:

# ```python

# FILE\_PATH = r"C:\\YOUR\\PATH\\clay\_create\\gh\_state.json"
...
# ├── gh\_state.json       # Auto-generated — shared state between Flask and GH
...
# | Flask → Grasshopper | File | — | `gh\_state.json` updated on every event |
